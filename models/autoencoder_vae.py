...
class Decoder(nn.Module):
  def __init__(self, latent_dim, out_dim, n_layers, layer_size):
    ...
    for i in range(n_layers + 1):
      if i == 0: # first layer
        dec.append(nn.Linear(latent_dim, layer_size))
        dec.append(nn.ReLU())
...
class AutoEncoder(nn.Module):
  def __init__(self, in_dim, latent_dim, n_layers_enc, layer_size):
    super(AutoEncoder, self).__init__()
    self.encoder = Encoder(in_dim, latent_dim, n_layers_enc, layer_size)
    self.decoder = Decoder(latent_dim, in_dim, n_layers_enc, layer_size)
...
class VAE(nn.Module):
  def __init__(self, in_dim, latent_dim, n_layers, layer_size):
    ...
    self.decoder = Decoder(latent_dim, in_dim, n_layers, layer_size)
